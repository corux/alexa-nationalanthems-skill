...
describe("AMAZON.LoopOnIntent and AMAZON.LoopOffIntent", () => {
  ...
  test("Should remember loop mode, before playback is started", async () => {
    let result = await alexa.request().intent("AMAZON.LoopOnIntent").send();

    expect(result.response.outputSpeech.ssml).toContain(
      "audio.loop-on-prepare"
    );
  ...
  test("Should allow changing loop mode, before playback is started", async () => {
    ...
    result = await alexa.request().intent("AMAZON.LoopOffIntent").send();
  ...
  test("Should allow activating loop mode during playback", async () => {
    ...
    result = await alexa.request().intent("AMAZON.LoopOnIntent").send();
  ...
  test("Should loop current anthem", async () => {
    const result = await alexa
      .request()
      .audioPlayer("AudioPlayer.PlaybackNearlyFinished", "DEU:1:0", 0)
      .send();
