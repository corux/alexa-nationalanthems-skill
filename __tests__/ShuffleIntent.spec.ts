...
describe("AMAZON.ShuffleOnIntent and AMAZON.ShuffleOffIntent", () => {
  ...
  test("Should remember shuffle mode, before playback is started", async () => {
    let result = await alexa.request()
      .intent("AMAZON.ShuffleOnIntent").send();
    ...
    expect(result.response.directives[0].audioItem.stream.token).toBe("USA:0:1");
  });
  ...
  test("Should allow changing shuffle mode, before playback is started", async () => {
    ...
    result = await alexa.request()
      .intent("AMAZON.ShuffleOffIntent").send();
  ...
  test("Should shuffle random anthems", async () => {
    const result = await alexa.request()
      .audioPlayer("AudioPlayer.PlaybackNearlyFinished", "DEU:0:1", 0).send();
